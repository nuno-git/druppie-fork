...
// + per-node "fixed" flag). We translate that into ELK's input format,
// run the layered algorithm with ORTHOGONAL edge routing and
// partition-by-layer so Business sits above Application sits above
// Technology, then return the laid-out coordinates back. The writer
// patches them into the .archimate XML before serialising.
//
...
app.post('/layout', async (req, res) => {
  try {
    ...
    res.json({ success: true, nodes: laidOut })
  } catch (err) {
