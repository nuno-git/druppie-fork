...
class TranslationService:
    ...
    Notification state is stored in the ``session_notifications`` DB table
    so it is shared across all backend workers.
    """
    ...
    def mark_notified(cls, session_id: str) -> bool:
        ...
        State is stored in the DB so it is shared across all workers.
        """
        from druppie.db.database import SessionLocal
        from druppie.db.models.session_notification import SessionNotification

        db = SessionLocal()
        try:
            existing = (
                db.query(SessionNotification)
                .filter(SessionNotification.session_id == session_id)
                .first()
            )
            if existing:
                return False
            db.add(SessionNotification(session_id=session_id))
            db.commit()
            return True
        except Exception:
            db.rollback()
    ...
    def clear_session(cls, session_id: str) -> None:
        ...
        db = SessionLocal()
        try:
            db.query(SessionNotification).filter(
