...
docker compose exec druppie-db psql -U druppie -d druppie -c "\dt" 2>/dev/null || warn "Could not query tables"

echo ""
log "Row counts per table:"
docker compose exec druppie-db psql -U druppie -d druppie -c "
SELECT
    schemaname,
...
docker compose exec druppie-db psql -U druppie -d druppie -c "SELECT id, username, email FROM users LIMIT 5;" 2>/dev/null || warn "Could not query users"

if [ -n "$SESSION_ID" ]; then
    echo ""
    log "Sessions table content:"
    docker compose exec druppie-db psql -U druppie -d druppie -c "SELECT id, title, user_id, status, created_at FROM sessions LIMIT 5;" 2>/dev/null || warn "Could not query sessions"

    echo ""
    log "Messages table content:"
    docker compose exec druppie-db psql -U druppie -d druppie -c "SELECT id, session_id, role, content FROM messages LIMIT 5;" 2>/dev/null || warn "Could not query messages"
fi
