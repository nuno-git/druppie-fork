...
```bash
# pi_agent TypeScript (if changed)
cd pi_agent && npx tsc

# Backend container (Python or pi_agent changes)
...
curl -s -X POST "http://localhost:10222/api/sessions/$SESSION_ID/retry-from/$AGENT_RUN_ID" \
  -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
  -d '{"planned_prompt": "Use execute_coding_task_pi with flow=tdd to create a hello world page"}'

# 6. Monitor (poll every 10s, max 4min)
curl -s "http://localhost:10222/api/sessions/$SESSION_ID" -H "Authorization: Bearer $TOKEN"
docker compose logs --tail=30 druppie-backend-dev 2>&1 | grep -E "pi_agent|error"

# 7. Check pi_agent run result
curl -s "http://localhost:10222/api/pi-agent-runs/by-tool-call/$TOOL_CALL_ID" -H "Authorization: Bearer $TOKEN"
# Look for: status, total_events, events array, summary
...
- `setup-yaml-flow-hello-world` — Full pipeline to pending developer (best for `execute_coding_task_pi`)
...
6. Watch the pi_agent run appear with live events
7. Stop if failing — click cancel on the session (also stops pi_agent)
8. Verify the result
