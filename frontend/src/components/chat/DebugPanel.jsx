...
import { ChevronDown, ChevronRight, Copy, Check, Bot, Wrench, Brain, Clock } from 'lucide-react'

/**
 * Simplified Debug Panel - Single overview with expandable agents
 */
export default function DebugPanel({ trace, isLoading, error }) {
  const [expandedAgents, setExpandedAgents] = useState(new Set())
  const [expandedLLMCalls, setExpandedLLMCalls] = useState(new Set())
  const [copied, setCopied] = useState(false)

  if (isLoading) return <div className="p-4 text-gray-500">Loading trace...</div>
  if (error) return <div className="p-4 text-red-500">Error: {error.message}</div>
  if (!trace) return <div className="p-4 text-gray-500">No trace data</div>

  const events = trace.events || []
  const llmCalls = trace.raw_llm_calls || trace.llm_calls || []
  ...
  const agents = useMemo(() => {
    ...
    events.forEach(event => {
      ...
      if (eventType.includes('agent_start') && agentId) {
      ...
      if (eventType.includes('agent_complete') && agentId && agentMap.has(agentId)) {
    ...
    llmCalls.forEach(call => {
      ...
      if (agentId) {
        ...
        const toolCalls = call.response?.tool_calls || []
  ...
  // Calculate totals
  const totalTokens = llmCalls.reduce((sum, c) => sum + (c.usage?.total_tokens || 0), 0)
  ...
  const generateTraceText = () => {
    ...
    agents.forEach((agent, idx) => {
      ...
      agent.llmCalls.forEach((call, i) => {
        text += `  LLM Call ${i + 1}: ${call.model} (${call.usage?.total_tokens || 0} tokens, ${call.duration_ms}ms)\n`
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
      ...
      <div className="divide-y divide-gray-100">
        {agents.length === 0 ? (
          <div className="p-4 text-gray-500 text-sm">No agents executed yet</div>
        ) : (
          agents.map((agent) => (
