...
**Progress:** 6 tested / 4 skipped / 21 total (14 local, 7 API).
...
| Gemma-4-26B-A4B | `google/gemma-4-26B-A4B` | 26B (MoE, ~A4B active) | ⛔ Skipped | -- | -- | -- | -- | -- | -- | -- | Skipped -- Attempted in-cluster 2026-07-07 -- the vLLM InferenceService came up but ALL 20 scenarios errored (context/generate/latency/tool-call/stress); no usable numbers. Root cause: launched with qwen's serving args/profile -- gemma needs its OWN vLLM profile. Evidence kept in report-FAILED-all-errors.txt. (backlog only (no registered CR); fit informational. No confirmed nvidia/*-NVFP4 build; kept as the bf16 HF repo (already fits at bf16, so NVFP4 not required).) |
