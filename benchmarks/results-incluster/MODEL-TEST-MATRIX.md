...
Self-updating backlog of LLM benchmark candidates. Regenerated every sweep by `benchmarks/update_test_matrix.py` from `benchmarks/candidates.yaml`; a model is **Tested** once `results-incluster/<slug>/report.txt` exists, or **Skipped** when the sweep records a reason (a runtime `results-incluster/<slug>/SKIPPED.txt`, or a static `skip_reason` in `candidates.yaml` for models too large / needing 2 GPUs).

**Legend** -- ✅ Tested: benchmarked, metrics parsed from its `report.txt`. ⛔ Skipped: not benchmarked, reason in Notes (too large / needs a maintenance window / serving fast-failed). ⬜ To test: on the backlog, not yet attempted (`--` metrics).

**Progress:** 3 tested / 3 skipped / 21 total (14 local, 7 API).
...
| Qwen3.6-27B-NVFP4 | `nvidia/Qwen3.6-27B-NVFP4` | 27B (NVFP4) | ✅ Tested | 185 ms | 64.0 t/s | 16.1 s | 421 ms | 0.30 s | 5,400 ms | 0 | SERVED in prod by isvc qwen-27b (modelRef qwen3-6-27b); benchmarked IN PLACE, no scaling. In-cluster live 2026-07-06 (256K, cu129-nightly). |
...
| Gemma-4-26B-A4B | `google/gemma-4-26B-A4B` | 26B (MoE, ~A4B active) | ⬜ To test | -- | -- | -- | -- | -- | -- | -- | backlog only (no registered CR); fit informational. No confirmed nvidia/*-NVFP4 build; kept as the bf16 HF repo (already fits at bf16, so NVFP4 not required). |
...
| Gemma-4-E4B-it | `unsloth/gemma-4-E4B-it-GGUF` | E4B | ⬜ To test | -- | -- | -- | -- | -- | -- | -- | extreme-speed small variant; GGUF + SentencePiece tokenizer. UNSURE: gemma4 GGUF arch may be unsupported by vLLM -- expected to fast-fail with a captured reason |
