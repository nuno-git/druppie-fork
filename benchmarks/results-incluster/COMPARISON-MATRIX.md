...
10 registered model(s) tracked; 0 benchmarked from 0 result file(s). Models that are not (yet) benchmarked still appear, with a `Status / Note` explaining why (fits/pending, needs a maintenance window, or too large for this hardware).
...
| Qwen3.6-27B | 27B | bf16 ~54GB | -- | -- | -- | -- | -- | -- | -- | Pending -- currently served in prod. |
...
_No benchmarked models yet -- run the sweep to populate._

## Source files

_None -- matrix rendered from the known-models registry only._
