...
**Site allowlist:** Use `SHAREPOINT_ALLOWED_SITES` with the same three-mode convention as `ENTRA_ALLOWED_EMAILS`: `all` (no filtering), empty (block all, fail-closed default), or semicolon-separated site URLs. Filtering is enforced server-side on all operations by comparing the Graph site's `webUrl` against the allowlist. Verified site IDs are cached in-memory to avoid redundant Graph lookups; the cache stores URL-allowlist decisions (server-wide config), not per-user authorization (which is enforced by Graph via the OBO token on every call).
