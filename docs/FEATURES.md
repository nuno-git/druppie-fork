...
Both skills are backed by one platform-research document, `docs/research/002-llm-orchestration-in-apps.md`, which leads with the standard and keeps the framework survey as a considered-alternatives appendix. Each agent calls `invoke_skill(...)` itself when the signals match — the user doesn't have to ask.
