...
Each agent that needs file access gets its own sandbox container via a Sandbox MCP server. Agents sharing the same git scope share the same container. A warm pool of pre-started containers handles performance. The sandbox provides tools like `read_file`, `write_file`, `bash`, `push_changes`, and `submit_design_for_review`.
