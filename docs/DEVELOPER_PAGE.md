# Developer Page — Quick pi_agent Testing

## 1. Purpose
A dedicated page for quickly testing pi_agent flows without going through the full Druppie agent pipeline (router → BA → architect → planner → developer). Select a project, pick a flow, enter a task prompt, and execute directly.
...
├──────────────────┬──────────────────────────────────────────────────┤
│  ┌──────────────┐│  ┌──────────────────────────────────────────────┐│
│  │ Project      ││  │  Execute a pi_agent run to see results here  ││
│  │ [dropdown  ▼]││  │                                              ││
│  │              ││  │        [Code2 icon]                          ││
...
| /api/sessions/{id}/retry-from/{agentRunId} | POST | Trigger developer with pi_agent task |
| /api/pi-agent-runs/by-tool-call/{tcId} | GET | Get pi_agent events for LiveCard |
...
5. Poll GET /api/sessions/{id} every 2s for max 30s, looking for execute_coding_task_pi in agent run tool calls
6. Once toolCallId is found, set it → PiCodingRunLiveCard renders and starts polling /by-tool-call
7. Show status updates: "Creating session..." → "Starting pi_agent..." → "Running..." → "Completed"
...
- **pi_agent run fails**: PiCodingRunLiveCard shows failure state
- **Timeout polling**: Show "Timed out waiting for pi_agent to start" with retry
